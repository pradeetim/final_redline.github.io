...
Our project also explored the relationship between redlining grades and three health outcomes: diabetes, high cholesterol, and obesity. The results, derived from regression models for each health outcome, were statistically significant across all models, suggesting a strong relationship between redlining and these health conditions.

Obesity Model:
The results from the obesity model show a clear relationship between redlining grades and obesity. Specifically, individuals from areas with worse redlining grades (e.g., grades C and D) are more likely to have higher obesity outcomes. The intercept of 37.275 (p < 0.001) indicates a baseline level of obesity for individuals in grade A areas. However, when moving to red_gradeB, red_gradeC, and red_gradeD, the estimated effects were negative, with estimates of -2.775, -3.133, and -4.989, respectively, all with p-values less than 0.001. This trend suggests that, contrary to what might be expected, individuals from higher redlining grades may have lower obesity scores, potentially due to systemic barriers and less access to health resources that prevent the management of obesity.

Diabetes Model:
In the diabetes model, the relationship between redlining and diabetes outcomes is more directly significant. The intercept of 20.736 (p < 0.001) provides the baseline diabetes rate for individuals in grade A areas. For red_gradeB, red_gradeC, and red_gradeD, the estimates were 5.187, 6.770, and 7.974, respectively (all with p < 0.001). These results show that as the redlining grade worsens, diabetes prevalence increases significantly. This finding aligns with previous research suggesting that neighborhoods with poorer infrastructure, limited access to healthy food, and reduced access to healthcare services may exacerbate diabetes rates. The statistically significant p-values for all grades further support that redlining has a substantial effect on diabetes outcomes.

High Cholesterol Model:
In the high cholesterol model, the results indicate a similar trend, with redlining grades playing a significant role. The intercept for grade A was 9.247 (p < 0.001), while the estimates for red_gradeB, red_gradeC, and red_gradeD were 1.866, 3.108, and 2.337, respectively. While the relationship is positive, it is less pronounced than for diabetes. The coefficients show a gradual increase in cholesterol levels as the redlining grade worsens, and the p-values for all estimates were statistically significant (p < 0.05), further solidifying the link between redlining and high cholesterol.

In this extended analysis, we performed multivariable regression models that adjusted for additional covariates, including race (ph_white, ph_black, ph_asian), age (ph_elderly), disability (ph_disability), and employment status (ph_no_work). The goal of these models was to assess whether the relationships observed between redlining grades and health outcomes would hold after accounting for these important socio-demographic factors.

Obesity: After adjusting for other socio-economic factors, redlining grades are still associated with obesity, but the relationship becomes weaker. This suggests that redlining itself may not be the sole determinant of obesity, and other socio-economic factors, such as race and employment status, play a significant role.

Diabetes: The relationship between redlining grades and diabetes is robust, even after controlling for race, age, disability, and employment status. This indicates that redlining has a direct and significant impact on the prevalence of diabetes, likely due to limited access to healthcare, healthy food, and physical activity resources in redlined areas.

High Cholesterol: For high cholesterol, redlining continues to be a significant predictor, but the effect size is smaller than for diabetes. The adjusted model confirms that race and socioeconomic status are crucial in understanding how redlining impacts health outcomes, with individuals from certain racial backgrounds experiencing more severe effects.

These adjusted regression models confirm that redlining is a significant factor influencing health outcomes such as obesity, diabetes, and high cholesterol. The continued significance of redlining, even after adjusting for socio-economic factors, underscores the enduring effects of redlining on public health. This finding highlights the importance of addressing not only the direct effects of redlining but also the broader social determinants of health, including race, disability, and employment status, to mitigate health inequalities.
