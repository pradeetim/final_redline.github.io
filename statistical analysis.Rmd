...
### Desciptive Characteristics based on hrs quartiles
...
final_analysis |>
  ...
  summarise(
    ...
    avg_ph_no_work = mean(ph_no_work, na.rm = TRUE),
    sd_ph_no_work = sd(ph_no_work, na.rm = TRUE),
    avg_ph_1_work = mean(ph_1_work, na.rm = TRUE),
    sd_ph_1_work = sd(ph_1_work, na.rm = TRUE),
    avg_ph_2_work = mean(ph_2_work, na.rm = TRUE),
    sd_ph_2_work = sd(ph_2_work, na.rm = TRUE),
  ...
  pivot_wider(
    names_from = c(interval2020, stat), 
    values_from = value,              
    names_glue = "{interval2020}_{stat}"
  )
...
q1_vs_q4|>
 ...
 pivot_longer(cols = -interval2020, names_to = "variable", values_to = "value")|>
 pivot_wider(names_from = interval2020, names_prefix = "quartile_", values_from = value)|>
 mutate(
    mean_diff = quartile_4-quartile_1,
  )|>
...
uni_reg(q1_vs_q4, outcomes, "interval2020")|>
  filter(term != "(Intercept)")|>
  right_join(q1q4_desc, by="outcome")|>
  select(outcome, quartile_1, quartile_4, mean_diff, p.value)|>
...
```{r}
# univariate analysis
lm(ph_snap~interval2020,final_analysis)|>
  broom::tidy()

## multivariate model 1
lm(ph_snap~interval2020+minority+ph_disability,final_analysis)|>
  broom::tidy()|>
  knitr::kable(digits = 3)


##multivariate model 2
lm(ph_snap~interval2020+ph_disability+ph_white+ph_black+ph_asian+ph_1_work+ph_2_work,final_analysis)|>
  broom::tidy()|>
  knitr::kable(digits = 3)
```
