...
```{r}
leafletOutput("redlining_map", height = 600)
```

```{r, context="server"}
output$redlining_map <- renderLeaflet({
  # Filter data based on the input from the sidebar
...
output$health_map <- renderLeaflet({
  leaflet(health_spatial_data %>% filter(condition == input$health_indicator & borough == input$borough_choice)) %>%
    addProviderTiles(providers$CartoDB.Positron) %>%  
    addPolygons(
