...
health_data %>%
  ...
  group_by(red_grade) %>%
  mutate(
    obesity_avg = mean(obesity, na.rm = FALSE),
    diabetes_avg = mean(diabetes, na.rm = FALSE),
    highchol_avg = mean(highchol, na.rm = FALSE)
  ) %>%
  pivot_longer(cols = c(diabetes_avg, obesity_avg, highchol_avg), 
               names_to = "condition", values_to = "rate") %>%
  ggplot(aes(x = rate, fill = condition)) +
  geom_histogram(binwidth = 10, color = "black", alpha = 0.7, , position = "dodge"
                 ) +
  facet_wrap(~red_grade) +  # Create facets by red_grade
  labs(
  ...
  theme_minimal() +
  scale_fill_manual(values = c("diabetes_avg" = "blue", "obesity_avg" = "red", "highchol_avg" = "green"))
